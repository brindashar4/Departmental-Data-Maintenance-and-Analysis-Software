...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update guestlec set permission_path ='NULL'  where p_id = $cardId";
		...
		{
			$query = "Update guestlec set permission_path ='not_applicable'  where p_id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update guestlec set permission_path =' ".$targetName."'  where p_id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
                ...
                <form action="" method="POST" enctype="multipart/form-data" class="row">
					...
					<div class='second-reveal'>
					 <div class="form-group col-md-6">
                         ...
                         <label for="card-image">Permission Letter * </label>
