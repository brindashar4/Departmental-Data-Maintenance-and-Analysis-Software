...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update co_curricular set report_path ='NULL'  where co_curricular_ID = $cardId";
		...
		{
			$query = "Update co_curricular set report_path ='not_applicable'  where co_curricular_ID = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update co_curricular set report_path =' ".$targetName."'  where co_curricular_ID = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
