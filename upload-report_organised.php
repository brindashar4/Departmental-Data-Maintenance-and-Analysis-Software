...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update organised set Report_path ='NULL'  where A_ID = $cardId";
		...
		{
			$query = "Update organised set Report_path ='not_applicable'  where A_ID = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update organised set Report_path =' ".$targetName."'  where A_ID = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
