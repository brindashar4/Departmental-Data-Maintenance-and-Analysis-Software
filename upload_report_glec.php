...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update guestlec set report_path ='NULL'  where p_id = $cardId";
		...
		{
			$query = "Update guestlec set report_path ='not_applicable'  where p_id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update guestlec set report_path =' ".$targetName."'  where p_id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
