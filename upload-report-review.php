...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update paper_review set report_path ='NULL'  where paper_review_ID = $cardId";
		...
		{
			$query = "Update paper_review set report_path ='not_applicable'  where paper_review_ID = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update paper_review set report_path =' ".$targetName."'  where paper_review_ID = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
