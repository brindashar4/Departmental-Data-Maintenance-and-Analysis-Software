	...
	if(isset($_POST['add']))
	{
		...
		{	 	
			for($i=0;$i<$count;$i++)	
			{
				...
				$val = array($id,$f_id[$i],$ind[$i],$city[$i],$purpose[$i],/*$ivdate[$i],*/$t_audience[$i],$staff[$i],$permission[$i],$report[$i],$certificate[$i],$attendance[$i],$from[$i],$to[$i],$part[$i],$ivtype[$i],$details[$i]);
				if($id!=-999)
				{				
            ...
            <div class="col-md-6">
              <!-- general form elements -->
				...
				<?php
					...
					<div class="form-group col-md-8">
						<input type = "radio" name = "sponsors[]" id= "sponsored" value= "sponsored" onClick= "javascript:yesnoCheck();">
						<label for= "sponsored">Sponsored </label>
						<input type = "radio" name = "sponsors[]" id= "nsponsored" value= "not sponsored" onClick= "javascript:yesnoCheck();">
