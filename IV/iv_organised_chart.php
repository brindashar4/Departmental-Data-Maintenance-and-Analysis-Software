...
if($_SESSION['username'] == ('hodextc@somaiya.edu') || $_SESSION['username'] == ('member@somaiya.edu') || $_SESSION['username'] == ('hodcomp@somaiya.edu') )
  {
		...
		$query1 = "SELECT MAX(t_from) from iv_organized ";
$result1 = mysqli_query($conn,$query1);
while($row =mysqli_fetch_assoc($result1)){
    $maxEndDate = $row['MAX(t_from)'];
}

/*$query2 = "SELECT MIN(durationf) from iv_attended WHERE Fac_ID ='".$_SESSION['Fac_ID']."'; ";
...
while($startYear != $endYear) {
	...
	$query = "SELECT COUNT(*) FROM iv_organized WHERE t_from >= '$date1' AND t_to <= '$date2'";
  ...
  else
  {
	  ...
	  $query1 = "SELECT MAX(t_from) from iv_organized WHERE f_id ='".$_SESSION['Fac_ID']."'; ";
$result1 = mysqli_query($conn,$query1);
while($row =mysqli_fetch_assoc($result1)){
    $maxEndDate = $row['MAX(t_from)'];
}
...
while($startYear != $endYear) {
	...
	$date1 = $startYear.'-07-01';
	$date2 = $temp_year.'-06-30';
	$query = "SELECT COUNT(*) FROM iv_organized WHERE t_from >= '$date1' AND t_to <= '$date2' AND f_id = $fid ";
	$result = mysqli_query($conn,$query);
	while($row = mysqli_fetch_assoc($result)){
    	$temp_count = $row['COUNT(*)'];
    	array_push($year, $startYear.'-'.substr($temp_year,2,4));
			...
			<div class="col-md-6">
              ...
              <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title">No. of Organised lectures</h3>
