...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update guestlec set attendance_path ='NULL'  where p_id = $cardId";
		...
		{
			$query = "Update guestlec set attendance_path ='not_applicable'  where p_id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
		...
		else if($_POST['applicable'] == 1)
		{
			if(isset($_FILES['image']))
			{
			  ...
			  $targetName="Attendance/".$_POST['id'].".".$fileExt;  
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update guestlec set attendance_path =' ".$targetName."'  where p_id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
...
{
?>
      <a href="view_organised_lec.php" type="button" class="btn btn-warning btn-lg">Cancel</a>
<?php
}
?>			 
