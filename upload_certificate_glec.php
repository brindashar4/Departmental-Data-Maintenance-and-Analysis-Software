...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update guestlec set certificate_path ='NULL'  where p_id = $cardId";
		...
		{
			$query = "Update guestlec set certificate_path ='not_applicable'  where p_id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			...
			if(isset($_FILES['image']))
			{
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update guestlec set certificate_path =' ".$targetName."'  where p_id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
...
<div class="col-md-12">
   ...
   <button name="insert-image" type="submit" class="btn btn-primary btn-lg">Insert</button>
 <div class="pull-right">                        
<?php 
	 if($_SESSION['username'] == ('hodextc@somaiya.edu') || $_SESSION['username'] == ('member@somaiya.edu') || $_SESSION['username'] ==('hodcomp@somaiya.edu'))
{ 
echo "<a href='menu.php?menu=4.php' type='button' class='btn btn-primary btn-lg'>Cancel</a>";
?>
